...
int ntapfuse_truncate(const char *path, off_t off) {
  ...
  initFileSize = initFileSize % BLOCK_SIZE != 0
                     ? (initFileSize / BLOCK_SIZE + 1) * BLOCK_SIZE
                     : initFileSize;
  ...
  if (off >= initFileSize) {
    ...
    usage = off > BLOCK_SIZE ? ((off - initFileSize) / BLOCK_SIZE) * BLOCK_SIZE
                             : BLOCK_SIZE - initFileSize;
  }
...
int ntapfuse_write(const char *path, const char *buf, size_t size, off_t off,
                   struct fuse_file_info *fi) {
...
int ntapfuse_fsync(const char *path, int datasync, struct fuse_file_info *fi) {
  if (datasync)
...
int ntapfuse_readdir(const char *path, void *buf, fuse_fill_dir_t fill,
  ...
  while ((de = readdir((DIR *)fi->fh)) != NULL) {
    ...
    if (fill(buf, de->d_name, &st, 0)) break;
  }
